...
import { ChatMessage, Player, Game, HostedGameOptions, Move, HostedGame, HostedGameToPlayer } from '../shared/app/models';
...
{
    /**
     * Entity that holds data and can be persisted.
     */
    private hostedGame: HostedGame;

    /**
     * Players in the game, in the good current order.
     */
    ...
    static hostNewGame(gameOptions: HostedGameOptions, host: Player, rematchedFrom: null | HostedGame = null): HostedGameServer
    {
        ...
        console.log(inspect(wrap(hostedGame).toObject(), false, 12, true));
    ...
    getPlayerStrict(index: PlayerIndex): Player
    {
        const player = this.players[index];

        if (!player) {
            throw new Error(`No player at index ${index}`);
        }
    ...
    getPlayerIndex(player: Player): null | number
    {
        const index = this.players
            .findIndex(p => p.publicId === player.publicId)
        ;
    ...
    reversePlayers(): void
    {
        if (2 !== this.players.length) {
            throw new Error('Cannot reverse players, need 2 players');
        }
    ...
    private async makeAIMoveIfApplicable(): Promise<void>
    {
        ...
        const player = this.getPlayerStrict(this.game.getCurrentPlayerIndex());
    ...
    private getLastPlayerMove(): null | Move
    {
        ...
        if (this.getPlayerStrict(i % 2 as PlayerIndex).isBot) {
            --i;
    ...
    {
        if (null === this.hostedGame || null === this.game || 'playing' !== this.getState() || 'number' !== typeof this.hostedGame.undoRequest) {
            return;
        }

        const player = this.getPlayerStrict(1 - this.hostedGame.undoRequest as PlayerIndex);
    ...
    private createAndStartGame(): void
    {
        ...
        this.io.to(this.gameRooms(true)).emit('gameStarted', this.hostedGame);
        this.io.to(this.gameRooms()).emit('timeControlUpdate', this.getPublicId(), this.timeControl.getValues());
    ...
    private affectPlayersColors(): void
    {
        ...
        if (null === this.hostedGame.gameOptions.firstPlayer) {
            ...
            if (null !== this.hostedGame.rematchedFrom && this.shouldAlternateColorsFromRematchedGame()) {
                ...
                const previousFirstPlayer = this.hostedGame.rematchedFrom.hostedGameToPlayers[0].player.id;
                const currentFirstPlayer = this.getPlayerStrict(0).id;
    ...
    private shouldAlternateColorsFromRematchedGame(): boolean
    {
        ...
        const previousPlayersIds = this.hostedGame.rematchedFrom.hostedGameToPlayers.map(hostedGameToPlayer => hostedGameToPlayer.player.id);
        const currentPlayersIds = this.players.map(player => player.id);
    ...
    playerAskUndo(player: Player): true | string
    {
        ...
        if (null !== this.hostedGame.undoRequest) {
            logger.notice('A player tried to ask undo but there is already an undo request', { hostedGameId: this.getPublicId(), player: player.pseudo, undoRequest: this.hostedGame.undoRequest });
        ...
        this.hostedGame.undoRequest = playerIndex;
    ...
    playerAnswerUndo(player: Player, accept: boolean): true | string
    {
        ...
        if (null === this.hostedGame.undoRequest) {
            logger.notice('A player tried to answer undo but there is no undo request', { hostedGameId: this.getPublicId(), player: player.pseudo, undoRequest: this.hostedGame.undoRequest });
            return 'there is no undo request';
        }

        if (this.hostedGame.undoRequest === playerIndex) {
            logger.notice('A player tried to answer his own undo request', { hostedGameId: this.getPublicId(), player: player.pseudo, undoRequest: this.hostedGame.undoRequest });
            return 'cannot answer own undo request';
        }

        const timeControlAfterUndo = recreateTimeControlAfterUndo(this.hostedGame, this.game.playerUndoDryRun(this.hostedGame.undoRequest as PlayerIndex).length, now);
        ...
        if (accept) {
            ...
            this.io.to(this.gameRooms()).emit('timeControlUpdate', this.getPublicId(), this.hostedGame.timeControl);
        }
    ...
    private canCancel(player: Player): true | string
    {
        ...
        if ('playing' !== this.getState() && 'created' !== this.getState()) {
    ...
    postChatMessage(chatMessage: ChatMessage)
    {
        ...
        this.hostedGame.chatMessages.add(chatMessage);
    ...
    private saveGameState(): void
    {
        ...
        if (null === this.hostedGame.gameData) {
            this.hostedGame.gameData = new Game();
        }

        wrap(this.hostedGame.gameData).assign(this.game.toData());
    }
    ...
    private savePlayersState(): void
    {
        for (let i = 0; i < this.players.length; ++i) {
            ...
            if (undefined === hostedGameToPlayer) {
                ...
                this.hostedGame.hostedGameToPlayers.add(hostedGameToPlayer);
            }
