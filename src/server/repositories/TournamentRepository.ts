...
export default class TournamentRepository
{
    ...
    private async createActiveTournament(tournament: Tournament): Promise<ActiveTournament>
    {
        ...
        // debounce to prevent deadlock when saving twice e.g when a game starts at same time another ends
        const saveDebounced = pDebounce.promise(() => this.save(tournament));

        // TODO still deadlock: create tournament, start it, iterate

        activeTournament.on('started', () => saveDebounced());
        activeTournament.on('gameStarted', () => saveDebounced());
        activeTournament.on('gameEnded', () => saveDebounced());
        activeTournament.on('ended', () => saveDebounced());
