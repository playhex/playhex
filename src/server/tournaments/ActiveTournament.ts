...
import { NotEnoughParticipantsToStartTournamentError, TournamentError } from './TournamentError.js';
import { TournamentEngineInterface } from './organizers/TournamentEngineInterface.js';
import { createGameOptionsForTournament, sortAndRankParticipants, tournamentMatchNumber } from '../../shared/app/tournamentUtils.js';
...
export class ActiveTournament extends TypedEmitter<TournamentEvents>
{
    ...
    /**
     * Called when this tournament is created, or loaded from memory.
     */
    ...
    findSubscriptionByPublicId(publicId: string): null | TournamentSubscription
    {
        return this.tournament.subscriptions
            .find(subscription => subscription.publicId === publicId)
            ?? null
        ;
    }
     ...
     */
    async playerSelfUnsubscribe(subscriptionPublicId: string): Promise<null | TournamentSubscription>
    {
        if ('created' !== this.tournament.state) {
            throw new TournamentError('Too late to unsubscribe, tournament started');
        }

        const subscription = await this.removeSubscription(subscriptionPublicId);
     ...
     * He may join again.
     */
    async hostKick(subscriptionPublicId: string): Promise<null | TournamentSubscription>
    {
        if ('created' !== this.tournament.state) {
     ...
     */
    async removeSubscription(subscriptionPublicId: string): Promise<null | TournamentSubscription>
    {
        const tournamentSubscription = this.tournament.subscriptions
            .find(subscription => subscription.publicId === subscriptionPublicId)
        ;

        if (!tournamentSubscription) {
            this.logger.notice('removeSubscription() called, but subscription in not found', {
