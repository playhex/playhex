...
export default class AdminController
{
    ...
    {
        const allSuccess = await this.tournamentsRepository.persistAllTournaments();
    ...
    @Post('/api/admin/tournaments/:publicId/iterate')
    async iterateTournament(
        @Param('publicId') publicId: string,
    ) {
        const activeTournament = this.tournamentsRepository.getActiveTournament(publicId);

        if (null === activeTournament) {
            throw new HttpError(400, 'This tournament is not loaded');
        }
