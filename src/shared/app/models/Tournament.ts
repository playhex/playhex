...
export default class Tournament implements TimeControlBoardsize
{
    ...
    /**
     * Free text where host can add any information, rules, ...
     */
    ...
    @Transform(({ value }) => 'fischer' === value.family
     ...
     * It can be:
     * - before if host starts it manually before,
     * - at date (or few milliseconds later) when started automatically,
     * - or later if host added a delay to auto start, or started manually.
     */
    @Column({ type: Date })
    @Expose({ groups: [GROUP_DEFAULT, 'tournament:create', 'tournament:edit'] })
    ...
    @Max(86400 * 365, { groups: [GROUP_DEFAULT, 'tournament:create'] })
     ...
     *      and host can still start it manually.
     * - Negative values: tournament will never start automatically.
     *      In this period, tournament is displayed as "imminent start",
     *      and host must start it manually.
     *
     * Used to give some control to host,
     * or to give a short period (like 10sec) in which tournament is displayed as "imminent start".
     */
    @Column({ type: Number, default: 0 })
    @Expose({ groups: [GROUP_DEFAULT, 'tournament:create'] })
    @IsInt({ groups: [GROUP_DEFAULT, 'tournament:create'] })
    @Max(86400 * 7, { groups: [GROUP_DEFAULT, 'tournament:create'] })
...
export const cloneTournament = (target: Tournament, source: Tournament): void => {
    ...
    target.timeControl = source.timeControl;
