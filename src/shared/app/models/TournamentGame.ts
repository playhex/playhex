...
export default class TournamentGame
{
    ...
    @Expose()
    @Type(() => Player)
    player1: null | Relation<Player>;

    @ManyToOne(() => Player)
    @Expose()
    @Type(() => Player)
    player2: null | Relation<Player>;
    ...
    /**
     * Round/number where the winner will go.
     * Example: "2.1"
     * Can be null e.g in Swiss format.
     */
    @Column({ type: String, length: 8, nullable: true })
    @Expose()
    winnerPath: null | string;

    /**
     * Round/number where the loser will go.
     * Example: "2.1"
     * Can be null e.g in Swiss format,
