...
export const defaultLadderRulesConfig: LadderRulesConfig = {
    ...
    liveProposalAnswerMs: 10 * MINUTE,
 ...
 * Defender must be active (not only connected or idle), else they won't answer in time.
 */
export const canProposeLive = (liveTimeControlType: TimeControlType, boardsize: number, defenderActive: boolean): null | LadderRefusalReason => {
    if (!isLive({ timeControlType: liveTimeControlType, boardsize })) {
        return 'live_time_control_not_live';
    }

    if (!defenderActive) {
        return 'defender_not_active';
    }
...
/**
 * Defender can accept to play live only if challenger is still active,
 * else challenger could lose on time while away.
 * Declining (correspondence) is always possible.
 */
export const canAcceptLive = (challengerActive: boolean): null | LadderRefusalReason => {
    return challengerActive
        ? null
        : 'challenger_not_active'
    ;
};
