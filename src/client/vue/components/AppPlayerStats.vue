...
let representedGames = 0;
let show = ref(0);

while (show.value < 3 && representedGames / totalGames < 0.70) {
    representedGames += topBoardsizes[show.value].playedGames;
    ++show.value;
}
</script>
    ...
    <div class="row">
        ...
        <div class="col-6 col-sm-6">
            ...
            <ul class="lead list-inline">
                <li v-for="{ boardsize } in topBoardsizes.slice(0, show)" :key="boardsize" class="list-inline-item">
                    {{ boardsize }}
                </li>
            </ul>
        </div>
