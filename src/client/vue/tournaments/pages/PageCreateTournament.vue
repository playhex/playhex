...
const tournament = ref(new Tournament());
...
tournament.value.checkInAt = new Date(tournament.value.startsAt.valueOf() - 10 * 60 * 1000);
...
/**
 * Check-in time relative to startsAt.
 * 1000 means check-in begins 1 seconds before tournament starts.
 */
const checkInRelative = ref<number>(10 * 60 * 1000);

watchEffect(() => {
    tournament.value.checkInAt = new Date(tournament.value.startsAt.valueOf() - checkInRelative.value);
});

const createTournament = (): void => {
    apiPostTournament(tournament.value);
};
</script>
