...
<template>
    <div class="container-fluid my-3">
        <template v-if="tournament">
            ...
            <template v-if="'ended' === tournament.state">
                <h2>Leaderboard</h2>

                <ul>
                    <li
                        v-for="participant in tournament.participants.sort((a, b) => (a.rank ?? 0) - (b.rank ?? 0))"
                        :key="participant.player.publicId"
                    >
                        {{ participant.rank ?? '-' }} - <strong>{{ participant.player.pseudo }}</strong>: {{ participant.score ?? '-' }} points
                    </li>
                </ul>
            </template>
