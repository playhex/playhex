...
export const useTournamentBracketsHelpers = (tournament: Tournament) => {
    ...
    return {
        ...
        */
        ...
        /**
         * Creates an arrow between two tournament games.
         *
         * @param from "1.1"
         * @param to "2.1"
        ...
        {
            const startElement = document.querySelector(`.brackets-match-${from.replace('.', '-')}`) as HTMLElement;
            const endElement = document.querySelector(`.brackets-match-${to.replace('.', '-')}`) as HTMLElement;
        ...
        drawAllArrows(tournament: Tournament, loserArrows = false): void
        {
            for (const tournamentGame of tournament.games) {
                const { winnerPath, loserPath } = tournamentGame;

                // Do not show arrow next to last game to prevent layout overflow and having an ugly horizontal scroll
                if (tournamentGame.round >= rounds.length) {
                    continue;
                }

                if (winnerPath) {
                    const line = this.drawArrow(
                        tournamentGame.round + '.' + tournamentGame.number,
                        winnerPath,
                        'win',
                    );
