...
const minutes = (ms: number): number => Math.round(ms / MINUTE);
</script>
...
<template>
    <div class="container my-3">
        ...
        <ul>
            ...
            <li>
                The challenger may propose to play live instead, only if the defender is currently active on PlayHex. The defender may only accept or decline this optional live format: if they accept, the live game starts, if they decline, the correspondence game starts.
                The defender has {{ minutes(config.liveProposalAnswerMs) }} minutes from when the challenge is sent to answer. Without an answer, the correspondence game starts.
                The defender can accept only while the challenger is still active, so the challenger does not lose on time while away.
        ...
        <ul>
            ...
            <li>Expired live proposals are processed every minute, and removals for inactivity every few minutes, not at the exact second.</li>
        </ul>
    </div>
</template>
