...
import { defaultLadderRulesConfig, getIncomingSlotsChoices, getLiveProposalExpiry, getMinIncomingSlots } from '../../../../shared/app/ladder/ladderRules.js';
...
import { IconExclamationTriangleFill, IconLightningChargeFill } from '../../icons.js';
...
/**
 * Live proposals I must answer
 */
const liveProposalsToAnswer = computed((): LadderChallenge[] => (props.me?.runningChallenges ?? [])
    .filter(c => c.state === 'pending_live' && c.defender.publicId === loggedInPlayer.value?.publicId),
);
        ...
        <div class="card-body">
            ...
            <template v-else-if="me">
                ...
                <template v-else-if="me.ladderPlayer && position !== null">
                    ...
                    <AppLadderSlots :player="me.ladderPlayer.player" :challenges="myIncomingChallenges" :total="me.incomingTotal" :seatsIfWon="me.seatsIfWon" />
                ...
                <!-- Live proposals to answer -->
                <div v-for="challenge in liveProposalsToAnswer" :key="challenge.publicId" class="alert alert-info">
                    <p class="mb-1">
                        <IconLightningChargeFill />
                        {{ $t('ladder.live_proposal', {
                    ...
                    <p class="small mb-2">{{ $t('ladder.live_proposal_expires', { date: formatDistanceToNowStrict(getLiveProposalExpiry(challenge.createdAt), { addSuffix: true }) }) }}</p>
                    <button type="button" class="btn btn-sm btn-success me-2" :disabled="loading" @click="answerLive(challenge, true)">{{ $t('ladder.accept_live') }}</button>
                    <button type="button" class="btn btn-sm btn-outline-secondary" :disabled="loading" @click="answerLive(challenge, false)">{{ $t('ladder.decline_live') }}</button>
                </div>
